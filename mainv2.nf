...
params.familyMode = params.containsKey('familyMode') ? params.familyMode.toString().toBoolean() : false
params.familySourceRoot = params.containsKey('familySourceRoot') ? params.familySourceRoot : '/ennis/projects/Research_Project-MRC147594/genome_sequencing'
params.familyBaseDir = params.containsKey('familyBaseDir') ? params.familyBaseDir : '/ennis/projects/Research_Project-MRC147594/genome_sequencing/families'
params.familyName = params.containsKey('familyName') ? params.familyName : null
params.familyTrios = params.containsKey('familyTrios') ? params.familyTrios : null
params.skip_hla = params.containsKey('skip_hla') ? params.skip_hla.toString().toBoolean() : false

def requestedFamilyIds = (params.containsKey('familySampleIds') ? params.familySampleIds : (params.containsKey('familySamples') ? params.familySamples : params.samples))
def requestedFamilyList = requestedFamilyIds ? requestedFamilyIds.toString().split(/[,\s]+/).collect { it.trim() }.findAll { it } : []
def defaultFamilyName = params.familyName ?: (requestedFamilyList ? requestedFamilyList.join('-') : null)

if( params.familyMode ) {
    if( !defaultFamilyName ) {
        throw new IllegalArgumentException("Family mode requires --familySampleIds")
    }
    params.batchDir = "${params.familyBaseDir}/${defaultFamilyName}"
    params.control = "${params.batchDir}/control"
} else {
    params.batchDir = params.containsKey('batchDir') ? params.batchDir : '.'
    params.control = params.containsKey('control') ? params.control : "${params.batchDir}/control"
}
...
include { buildStatusById; parseLineToMeta; parseLineToTuple; parseLineToTupleSpring; makeParsedLines; buildFamilyModeBatch; resolveExistingFile } from './lib/helpers.nf'

def familyBatchBootstrap = params.familyMode ? buildFamilyModeBatch() : null

workflow {

      def effectiveBatchDir = familyBatchBootstrap ? familyBatchBootstrap.batchDir : params.batchDir
      def effectiveControl = familyBatchBootstrap ? familyBatchBootstrap.control : params.control

      if( familyBatchBootstrap ) {
          println "\n=== FAMILY MODE ==="
          println "family batch : ${familyBatchBootstrap.batchDir}"
          println "family ids   : ${familyBatchBootstrap.samples.join(', ')}"
          println "family trios : ${familyBatchBootstrap.trios.collect { it.join('-') }.join(', ') ?: 'none'}"
          println "===================\n"
      }

      spring_dir = file(effectiveBatchDir + "/spring")
      ...
      def ch_control = file(effectiveControl)
      ...
      def statusById = buildStatusById(rowsForStatus, effectiveBatchDir)
      ...
      if (spring_dir.isDirectory() && !params.forcefq) {
        def springRows = controlLines
            .collect { parseLineToTupleSpring(it, effectiveBatchDir) }   // [ id, platform, sex, family, trio, flowcell, laneCount, famSampleCount, spring ]
            ...
            .map { parseLineToTuple(it, effectiveBatchDir) }
      ...
      def ch_single_sample_vcf_existing = Channel
        .from(metaById.keySet().toList())
        .filter { id -> !statusById[id].snv_needed }
        .map { id ->
            def meta = metaById[id]
            tuple(
                meta.id,
                ...
                meta.family,
                meta.famSampleCount,
                resolveExistingFile("${effectiveBatchDir}/r04_vcfs/${id}.vcf.gz"),
                resolveExistingFile("${effectiveBatchDir}/r04_vcfs/${id}.vcf.gz.csi")
            )
        }

      SNV_INDEL_CALLING.out.single_sample
        .mix(ch_single_sample_vcf_existing)
