    ...
    script:
      ...
      for (( i=0;            i < \${#ids[@]}; i++ )); do       
        for (( j=i+1;      j < \${#ids[@]}; j++ )); do  
           java -Xmx9g -XX:ParallelGCThreads=4 -XX:ConcGCThreads=4 -cp ${params.javaDir}:${params.gatkJar} VcfToHomozygosity7 ${vcf} -p \${ids[i]} \${ids[j]} > \${ids[i]}_\${ids[j]}_homozygosity.csv
      done
      done    
      """
}