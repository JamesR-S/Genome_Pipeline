...
process cleanCallContamination {
    ...
    input:
      tuple val(sample.name), file(bam) from realignedBams
    output:
      tuple val(sample.name), file("${sample.name}_cleanCall.csv") into cleanCallOutput
    script:
    """
...
process cleanCallContaminationSmall {
    ...
    input:
      tuple val(sample.name), file(bam) from realignedBams
    output:
      tuple val(sample.name), file("${sample.name}_cleanCall_small.csv") into cleanCallSmallOutput
    script:
    """
